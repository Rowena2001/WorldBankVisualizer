...
/**
 * Creates an object including all selections made by the user
 * runs all selection error checks
 * @author Group 15
 */
public class Selection {
	...
	/**
	 * creates list of valid viewers for the drop down menu
	 * @param filename
	 */
	...
	/**
	 * initializes the database of valid country and analysis combinations 
	 * @param fileName
	 */
	...
	/**
	 * gets the available list of countries after analysis type is selected to populate 
	 * the countries drop down menu
	...
	public ArrayList<String> availableCountry(){
		...
		//if the database with country and analysis combinations is empty then initialize it
		if (comboDB == null) {
			comboDB = new HashMap<String, HashMap<String, int[]>>();
			initializeComboDB(DBFILE);
		}
		
		//searching through each countries available analysis types
		for(int i = 0; i < countryName.size(); i++) {
			if (comboDB.containsKey(countryName.get(i).get(4))) {

				HashMap <String, int[]> tempCountry = comboDB.get(countryName.get(i).get(4));
				int counter = 0;
				//adding countries valid for analysis type
				for(int k = 0; k < selectedAnalysis.size(); k++) {
					if (tempCountry.containsKey(selectedAnalysis.get(k))) {
						if(tempCountry.get(selectedAnalysis.get(k)) != null)	
							counter++;
	...
	/**
	 * sets the start year of the selection
	 * @param start
	 */
	...
	/**
	 * sets the end year of the selection
	 * @param end
	 */
	...
	/**
	 * sets the start and end year of the selection from the MainUI
	 * @param start
	 * @param end
	 */
	...
	/**
	 * set the instance of the analysis type list
	 * @param title
	 */
	...
	/**
	 * tells the ViewerFactory to create a viewer object
	 * @param name
	 * @return Viewers
	 */
	...
	/**
	 * returns the titles for the analysis object
	 * @return titles
	 */
	...
	/**
	 * sets the full name of the country chosen
	 * @param country
	 */
	...
	/**
	 * gets the current AnalysisType object
	 * @return AnalysisType
	 */
